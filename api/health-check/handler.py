...
def check(name, url, method='GET', expected_status=200, body=None, min_size=0, headers=None):
    """Run a single health check using urllib (no external deps)."""
    start = time.time()
    try:
        ...
        size_ok = len(content) >= min_size if min_size > 0 else True

        if status_ok and size_ok:
            if elapsed > 5000:
                return {'name': name, 'status': 'warn', 'responseTime': elapsed,
                        'details': f'{status_code} OK but slow ({elapsed}ms)'}
            return {'name': name, 'status': 'pass', 'responseTime': elapsed,
                    'details': f'{status_code} OK, {len(content)} bytes'}
        elif status_ok and not size_ok:
            return {'name': name, 'status': 'warn', 'responseTime': elapsed,
                    'details': f'{status_code} OK but only {len(content)} bytes (expected >={min_size})'}
        else:
        ...
        if e.code == expected_status:
            return {'name': name, 'status': 'pass', 'responseTime': elapsed,
                    'details': f'{e.code} (expected)'}
        return {'name': name, 'status': 'fail', 'responseTime': elapsed,
                'details': f'HTTP {e.code} (expected {expected_status})'}
